MOB QUEST v163 - EVENT QUEST / FIGURE UPDATE

index.html を既存のゲーム素材と同じ階層に配置してください。
更新の詳細は CHANGELOG_v163.txt、確認結果は REGRESSION_REPORT_v163.txt を参照してください。
...
=== v160 FEEDBACK FIX ===
- 魔王城のAREA 1〜4を常時表示する進行レールを追加し、AREA 2 / AREA 3を含む4エリア定義を保護。
- 必要経験値の調整を報酬計算から分離し、敵の獲得EXPが連動して増えないよう修正。レベルアップ必要量は基準値の1.05倍。
- 必殺技はMP0を維持しつつ、攻撃系威力を6%だけ抑えて使用頻度上昇分を調整。
- GAME_ASSET_VERSION / 表示版を v160 に更新。


=== v161 READING BOOK EVENT FIX ===
- レコードルームの演出中は中央の読みかけの本全体を非表示にし、登場タイミングでのみ操作可能に変更。
- 読みかけの本AREA4で怪人ボスの表示を復元し、AREA3の退場演出の残留スタイルを除去。
- モブナビ戦後の怪人ボス単独表示、決戦前の手動パーティー編成を実装。怪人ボスは編成から選択でき、必殺技CTを全て0（最大）で開始。
- 「やれやれ、面倒だな」のナビ表示と吸収→モブナビマスター変身演出を安定化。
- モブナビ画像を boss/53.png に更新。
- GAME_ASSET_VERSION / 表示版を v161 に更新。


=== v163 NAVI FORM FIX ===
- v161で通常のモブナビまで変身後の画像にしてしまった指定を修正。
- 初対面・変身前の会話・勇者との単独戦：boss/52.png。
- 吸収演出後のモブナビマスター・決戦：boss/53.png。
- 内部/表示バージョンをv163に更新。
