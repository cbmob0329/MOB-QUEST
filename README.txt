MOB QUEST playable core v68

825追記・冒険イベント最新稿・ボス最新稿反映
- キャンプをAREAごとの「テント / 椅子 / ドリンク」個別1回制へ修正。パーティー機能は常時利用可能
- キャンプ/探索バナーをエリア別カラー木目へ変更。テント/椅子は暗転+ZZZ演出、確認/回復表示は白背景ナレーション
- 持ち物の説明からモブピンクを外し、アイテム使用対象のHP/MP・状態表示を大型化。健康/負傷/ダウンでカード色分け
- 酒場モブメープル紹介の改行・文字はみ出しを修正。未解放時は非表示を維持
- 部族村到着イベントのキャラクター縮小/モブジェシー位置ずれ再発を補正
- 砂漠IIのミラモブ台詞と四人衆画像参照を最新稿へ更新
- モブテツの長文台詞を短縮
- 戦闘中の入れ替え編成を戦闘終了後も正式パーティーへ継続
- 戦闘HUDを状態異常別カラー化。毒/やけどの継続ダメージ時に対象シェイク+頭上ダメージ表示
- ゴレマガーディ / ホクマウィング / ネオマニプールのフレーム順を更新し、通常魔法威力を再強化
- 長い持ち物/魔法/特技リストの閉じるヘッダーを追従表示
- 戦闘外のパーティーキャラクターをタップして、現在ステータス/習得済み魔法/特技/装備/必殺技を確認できる詳細ポップアップを追加
- 必殺技CTを戦闘間で保存・継続
- 王様報告イベント中のキャラクター再タップをロック
- お城メニューを2+3配置、黒いアイコンバナーを撤去し小型化
- 鍛冶屋を back/gonzo.png に変更。モブゴンゾー中央、HOME左小型、武器購入/メダル錬成をスクロール可能なポップアップ化
