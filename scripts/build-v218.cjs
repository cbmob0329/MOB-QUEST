...
const patch='// UPDATE_V218_BEGIN\nconst PARTY_SPEC_V221='+JSON.stringify(require('./party-spec-v221.cjs'))+';\nconst CATALOG_V218='+JSON.stringify(data)+';\n'+['update-v218.js','events-v218.js','soul-v218.js','integration-v218.js','update-v219.js','figures-v220.js','combat-v220.js','gacha-v220.js','update-v221.js','party-v221.js','presentation-v222.js','castle-report-v223.js','test-tools-v224.js'].map(f=>fs.readFileSync(path.join(root,'js',f),'utf8')).join('\n')+'\n// UPDATE_V218_END';
...
fs.writeFileSync(path.join(root,'css/style.css'),css.trim()+'\n/* UPDATE_V218_BEGIN */\n'+['update-v218.css','quest-presentation-v218.css','update-v219.css','update-v220.css','update-v221.css','presentation-v222.css','castle-report-v223.css','test-tools-v224.css'].map(f=>fs.readFileSync(path.join(root,'css',f),'utf8')).join('\n')+'\n/* UPDATE_V218_END */\n');
const html=path.join(root,'index.html');fs.writeFileSync(html,fs.readFileSync(html,'utf8').replace(/<div class="title-version">v\d+<\/div>/,'<div class="title-version">v224</div>'));
