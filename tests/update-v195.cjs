...
=======
ok(game.includes('window.__mobV195LilithIsolation=true'),'v195 runtime missing');
ok(game.includes("id='lilithIsolationV195'")||game.includes("id=\'lilithIsolationV195\'")||game.includes("ov.id='lilithIsolationV195'"),'white isolation layer missing');
ok(game.includes('パーティーを編成してください'),'formation narration missing');
ok(game.includes('data-lilith-isolation-start-v195'),'explicit formation button missing');
ok(game.includes('moneyFinalBridgeV195'),'Money fallback bridge missing');
ok(game.includes("splitOv.style.zIndex='2147483500'"),'split overlay is not promoted above isolation layer');
ok(game.includes("state.adventure.lilithSplitReadyV183=true"),'legacy duplicate-split guard missing');
ok(!game.includes('// UPDATE_V194_BEGIN'),'obsolete v194 runtime still compiled');
...
=======
ok(html.includes('<div class="title-version">v195</div>'),'title is not v195');
ok(html.includes('window.__mobV195LilithIsolation=true'),'inline runtime not synchronized');
console.log('update-v195 isolation/scope tests: OK');
>>>>>>> f51610d398df883925bdcd39b25a90c025784fd4
