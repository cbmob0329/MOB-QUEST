...
renderEventQuestsV163=function(){legendRenderBaseV218();const root=$('#trainingFeaturePanel'),panel=$('.event-panel-v163',root);if(!panel)return;
 ...
 if(eventViewV163!=='legend')return;panel.classList.add('legend-book-v218');panel.innerHTML=`<header><h2>レジェンドボス</h2><button data-legend-back>戻る</button></header><p>仲間になっているパーティー全員＋助っ人2名で戦うレイドボス！<br>全滅してもやり直せる！諦めずなんども挑もう！</p><p>再挑戦は残りHPを継続（最低30%）。</p><div class="book-pages-v218">${LEGENDS_V218.filter(legendOpenV218).map(q=>{const rec=legendRecordV218(q),ratio=rec.remaining===undefined?1:Math.max(.3,rec.remaining);return`<button data-legend="${q.key}"><img src="${q.image}" alt="${q.name}"><b>${q.name} Lv.${q.level}</b><meter min="0" max="1" value="${ratio}"></meter><small>次回HP ${Math.round(ratio*100)}%${rec.maxHp?` / ${Math.ceil(rec.maxHp*ratio).toLocaleString()} / ${rec.maxHp.toLocaleString()}`:''}${rec.cleared?' / CLEAR':''}</small></button>`;}).join('')}</div>`;
...
const legendFinishBaseV218=finishBattle;finishBattle=function(win){const b=state.battle;if(!b?.config?.legendV218)return legendFinishBaseV218(win);if(b.finished)return;b.finished=true;b.auto=false;b.legendWinV218=!!win;setCommandDisabled(true);const q=LEGENDS_V218.find(q=>q.key===b.config.legendV218),rec=legendRecordV218(q);let reward=null;if(win&&!rec.rewarded){reward=awardGachaFigureV115(figureByImageV96(`eventfig/${51+LEGENDS_V218.indexOf(q)}.png`));rec.rewarded=true;}saveLegendV218(b,win);const ov=eventOverlayV163();ov.innerHTML=`<div class="event-result-v163 legend-book-v218"><h2>${win?'LEGEND CLEAR!!':'また挑戦しよう！'}</h2><p>${win?'伝説の討伐を記録しました。':'残りHPを保存しました。次回は最低30%から再開します。'}</p>${reward?`<h3>初回確定報酬</h3><img style="width:120px;height:120px;object-fit:contain" src="${reward.f.image}" alt="${reward.f.name}"><p>${reward.f.name}${reward.converted?` / ルビー +${reward.ruby}`:reward.plus?` +${reward.plus}`:''}</p>`:''}<button data-return class="primary-btn">レジェンドボスへ戻る</button></div>`;ov.hidden=false;$('[data-return]',ov).onclick=returnLegendV218;};
...
const legendBackV218=$('#battleBackBtn').onclick;$('#battleBackBtn').onclick=async function(){if(!state.battle?.config?.legendV218)return legendBackV218?.();const b=state.battle;if(b.busy)return;if(await narrationDialog('HPを保存して挑戦を終了しますか？',[['終了する','yes'],['続ける','no','primary']])==='yes')returnLegendV218();};
