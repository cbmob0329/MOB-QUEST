...
function defaultMeta(){return{coins:12500,exp:{},inventory:{},drinkSets:{},weapons:{},medals:{},equipment:{},figures:{},figureEquipment:{},figureOrder:[],ultimateCooldowns:{},defeatedBosses:[],defeatedElites:[]};}
...
async function facilityTalk(text,speaker='モブピンク',image='play/02.png'){
  ...
  for(const line of lines){const chars=[...line];textEl.textContent=line;textEl.dataset.lineLength=String(chars.length);textEl.classList.toggle('musical-line',line.includes('♪'));
    textEl.style.setProperty('--facility-line-font',chars.length>32?'12px':chars.length>26?'13px':chars.length>21?'14px':chars.length>17?'15px':'17px');
    await new Promise(resolve=>{let ready=false;const timer=setTimeout(()=>ready=true,90);const next=e=>{if(!ready)return;e?.preventDefault?.();e?.stopPropagation?.();clearTimeout(timer);overlay.removeEventListener('pointerup',next,true);resolve();};overlay.addEventListener('pointerup',next,true);});await fixedDelay(120);
  }
  overlay.hidden=true;overlay.classList.remove('facility-line-talk','coach-dialogue');choices.innerHTML='';textEl.classList.remove('musical-line');textEl.style.removeProperty('--facility-line-font');delete textEl.dataset.lineLength;
}
...
async function openStoryScene(worldId,areaIndex=0,layout='default',extras=[]){const sc=$('#storyScene'),bg=storySceneBg(worldId,areaIndex),underParty=$('#adventureParty'),advScreen=$('#adventureScreen');setImage($('#adventureBg'),bg.bg,bg.fallback);$('#fieldEvent').hidden=true;if(advScreen)advScreen.classList.add('story-event-active');if(underParty)underParty.hidden=true;setAdventureVisualLoading(true);[...sc.classList].filter(c=>c.startsWith('story-world-')).forEach(c=>sc.classList.remove(c));sc.classList.add(`story-world-${worldId}`);sc.classList.remove('closing','shake','story-layout-party-left');if(layout==='partyLeftGuestRight')sc.classList.add('story-layout-party-left');storySceneExtras=Array.isArray(extras)?extras.filter(Boolean):[];$('#storyGuest').hidden=true;$('#storyGuestGroup').hidden=true;$('#storyGuestGroup').innerHTML='';$('#storyBubble').hidden=true;$('#storyNarration').hidden=true;storyTapResolve=null;sc.hidden=false;sc.style.visibility='hidden';await renderStoryParty();await nextPaint();sc.style.visibility='visible';setAdventureVisualLoading(false);await fixedDelay(240);}
...
function adventureAreaTheme(){const id=currentWorld()?.id||'';if(['grassland','grassland2'].includes(id))return'green';if(['desert','desert2'].includes(id))return'beige';if(['rural','rural2'].includes(id))return'sky';if(['magma','magma2'].includes(id))return'red';if(['neon','neon2'].includes(id))return'neon';if(id==='sea')return'blue';if(id==='tribe')return'brown';if(id==='demonCastle')return'redblack';if(id==='unfinishedBook')return'white';return'burgundy';}
function applyAdventureAreaTheme(){const theme=adventureAreaTheme();for(const el of [$('#campOverlay'),$('#exploreOverlay')])if(el){[...el.classList].filter(c=>c.startsWith('area-theme-')).forEach(c=>el.classList.remove(c));el.classList.add(`area-theme-${theme}`);}}
function renderAdventure(){
  const w=currentWorld(),area=currentArea(),bi=state.adventure.battleIndex||0,report=state.adventure.awaitingReport;
  $('#adventureStageTitle').textContent=state.adventure.completed?'魔王城までCLEAR':w.name;$('#adventureProgress').textContent=state.adventure.completed?'CLEAR':report?'王へ報告':`${area.name}　戦闘 ${bi+1}/3`;$('#areaName').textContent=state.adventure.completed?'魔王城までの冒険完了':report?`${w.name}・CLEAR`:`${w.name}・${area.name}`;
  const pending=state.adventure.pendingEncounter;$('#areaDescription').textContent=state.adventure.completed?'現在設定済みの草原～魔王城ルートをクリアしました。Lv上限が120になりました。':report?'次の地域へ進むには、お城の「王の間」で国王へ報告してください。':state.adventure.battleReady?(pending?.bossBattle?'強い気配がする。準備ができたら戦闘へ。':'敵の気配を感じる。何が現れるかは戦闘まで分からない。'):`探索 → バトルを3回行うと次のAREAへ進みます。3戦目は中ボス/ボスです。`;
  setImage($('#adventureBg'),area.bg,w.fieldFallback);setAdventureVisualLoading(true);
  const partyRoot=$('#adventureParty');partyRoot.innerHTML=state.party.slice(0,4).map(([id,lv])=>{const p=player(id);return p?`<button type="button" data-player-detail="${p.id}" aria-label="${p.name}のステータス"><img data-adventure-party-img src="${versionedPlay(p.image)}" alt="${p.name}" decoding="async"><span>${p.symbol}</span><small>Lv${lv}</small></button>`:'';}).join('');
  $$('[data-player-detail]',partyRoot).forEach(b=>b.onclick=e=>{e.stopPropagation();openPlayerDetail(b.dataset.playerDetail);});
  const blocked=!!report||state.adventure.completed||storyBusy;const btn=$('#fieldBattleBtn');btn.disabled=!state.adventure.battleReady||blocked;btn.classList.toggle('locked',btn.disabled);$('#fieldBattleHint').textContent=state.adventure.completed?'CLEAR':report?'王へ報告':state.adventure.battleReady?(pending?.bossBattle?'強敵の気配':'戦闘可能'):'探索が必要';$('#exploreBtn').disabled=state.adventure.battleReady||blocked;$('#campBtn').disabled=blocked;const campSmall=$('#campBtn small');if(campSmall)campSmall.textContent=report?'王へ報告してください':areaCampAllRestUsed()?'休憩3種 使用済 / パーティー利用可':'テント・椅子・ドリンク 各1回';bindImages($('#adventureScreen'));applyAdventurePartyScale();applyAdventureAreaTheme();
}
function currentAreaKey(){return `${state.adventure.worldIndex||0}:${state.adventure.areaIndex||0}`;}
function areaCampUsageRecord(){
  const raw=state.adventure.campUsed?.[currentAreaKey()];
  if(raw===true)return{tent:true,chair:true,drink:true};
  return raw&&typeof raw==='object'?raw:{};
}
function areaCampActionUsed(action){if(action==='party')return false;return !!areaCampUsageRecord()[action];}
function areaCampUsed(action=null){const r=areaCampUsageRecord();return action?!!r[action]:!!(r.tent||r.chair||r.drink);}
function areaCampAllRestUsed(){return ['tent','chair','drink'].every(k=>areaCampActionUsed(k));}
function markAreaCampUsed(action='tent'){if(!state.adventure.campUsed||typeof state.adventure.campUsed!=='object')state.adventure.campUsed={};const key=currentAreaKey(),raw=state.adventure.campUsed[key],rec=raw===true?{tent:true,chair:true,drink:true}:(raw&&typeof raw==='object'?{...raw}:{});rec[action]=true;state.adventure.campUsed[key]=rec;saveAdventure();}
...
function renderCampMain(){applyAdventureAreaTheme();const used={tent:areaCampActionUsed('tent'),chair:areaCampActionUsed('chair'),drink:areaCampActionUsed('drink')};$('#campTitle').textContent=`キャンプ / ${currentArea()?.name||''}`;$('#campUsageText').textContent='テント・椅子・ドリンクはこのAREAで各1回 / パーティーはいつでも使用可能';$('#campTentCount').textContent=tentCount();$('#campMainMenu').hidden=false;$('#campSubPanel').hidden=true;for(const action of ['tent','chair','drink']){const b=$(`[data-camp-action="${action}"]`,$('#campMainMenu'));if(b){b.classList.toggle('camp-used',used[action]);b.disabled=used[action];}}const party=$('[data-camp-action="party"]',$('#campMainMenu'));if(party){party.classList.remove('camp-used');party.disabled=false;}bindImages($('#campOverlay'));}
async function campFadeMessage(text,work,{sleep=false}={}){const f=$('#campFade');$('#campFadeText').textContent='';f.hidden=false;await nextPaint();f.classList.add('dark');await fixedDelay(520);if(sleep){$('#campFadeText').textContent='ZZZ・・・';await fixedDelay(820);}if(work)await work();$('#campFadeText').textContent=text;await fixedDelay(1050);f.classList.remove('dark');await fixedDelay(560);f.hidden=true;$('#campFadeText').textContent='';}
...
async function useCampTent(){if(areaCampActionUsed('tent'))return narrationDialog('このAREAではテントをすでに使用しました。');if(tentCount()<1)return narrationDialog('モブテントを所持していません！');const ans=await narrationDialog(`テントで休みますか？\nモブテントを1つ消費します`,[['はい','yes','primary'],['いいえ','no']]);if(ans!=='yes')return;if(!consumeItem('mob-tent',1))return;await campFadeMessage('テントで休み、パーティーは全回復した！',async()=>{healCampVitals(1);markAreaCampUsed('tent');saveCampCheckpoint();},{sleep:true});closeCamp();renderAdventure();}
async function useCampChair(){if(areaCampActionUsed('chair'))return narrationDialog('このAREAでは椅子をすでに使用しました。');const ans=await narrationDialog('椅子で休みますか？',[['はい','yes','primary'],['いいえ','no']]);if(ans!=='yes')return;await campFadeMessage('椅子で休み、パーティーのHPとMPが少し回復した！',async()=>{healCampVitals(.30);markAreaCampUsed('chair');saveCampCheckpoint();},{sleep:true});closeCamp();renderAdventure();}
...
function fieldTargetTone(x,st){if(x?.dead||x?.hp<=0)return'down';if(Number(x?.hp)<Number(st?.maxHp))return'damaged';return'healthy';}
...
function beginBattle(config){
  ...
  const partyList=(config.party||state.party).slice(0,10),vitals=config.questVitals|| (config.useAdventureVitals?state.adventure.vitals:null),allies=partyList.map(([id,lv])=>buildAlly(player(id),lv,vitals?.[id])),partySize=Math.min(4,allies.length);for(const a of allies)loadUltimateCooldowns(a);
...
function loadUltimateCooldowns(a){const stored=state.meta?.ultimateCooldowns?.[a.id];const base=(a.ults||[]).map((u,i)=>ultimateEffectiveCt(a,u,i));if(Array.isArray(stored))a.ultCooldowns=base.map((max,i)=>stored[i]==null?max:Math.max(0,Number(stored[i])||0));else a.ultCooldowns=base;}
function persistUltimateCooldownsFromBattle(){const b=state.battle;if(!b)return;if(!state.meta.ultimateCooldowns)state.meta.ultimateCooldowns={};for(const a of b.allies||[])if(a?.id&&Array.isArray(a.ultCooldowns))state.meta.ultimateCooldowns[a.id]=a.ultCooldowns.map(v=>Math.max(0,Number(v)||0));saveMeta();}
...
function allyStatusClass(a){if(a?.dead||a?.hp<=0)return'status-down';for(const k of ['poison','burn','sleep','paralyze','confuse'])if(Number(a?.status?.[k])>0)return`status-${k}`;return'';}
...
async function skillSprite(frames,target='enemy',fxSpec=null){
  ...
  try{
    ...
    if(mode==='earthLarge'){await show(0,240,'shake');await show(1,190);await show(2,190);await show(3,720,'fade');}
    else if(mode==='windLarge'){await show(0,180);await show(1,180);await show(2,180);await show(3,720,'fade');}
...
async function applyRoundDots(){
  ...
  for(const a of fieldAllies()){if(a.dead)continue;for(const k of ['poison','burn'])if(a.status[k]>0){const d=Math.max(1,Math.round(a.maxHp*.025));a.hp=Math.max(state.battle?.mode==='story'&&state.battle?.config?.scriptedImmortalParty?1:0,a.hp-d);a.status[k]--;renderBattle();await nextPaint();const el=$(`[data-ally-id="${a.id}"]`);el?.classList.add('status-damage-shake');floatNumber(d,'damage',a.id);await fixedDelay(260);el?.classList.remove('status-damage-shake');if(a.hp<=0){a.dead=true;notice(`${a.name} DOWN`,'danger');}else notice(`${a.name}に${k==='poison'?'毒':'やけど'}ダメージ！`,'status');}}
...
async function act(kind,payload){
  ...
  const usedUltIndex=kind==='ultimate'?a.ults.indexOf(payload):-1;advanceUltimateCooldowns(a,usedUltIndex);persistUltimateCooldownsFromBattle();
...
function persistBattlePartyOrder(){const b=state.battle;if(!b)return;const roster=[...new Set([...(b.mainIds||[]),...(b.superIds||[]),...(b.reserveIds||[])])];const globalIds=state.party.map(x=>x[0]);if(roster.length!==globalIds.length||!globalIds.every(id=>roster.includes(id)))return;const map=new Map(state.party.map(x=>[x[0],x]));state.party=roster.map(id=>map.get(id)).filter(Boolean);saveParty();state.training.party=state.party.map(x=>[...x]);}
...
function learnedUltimatesForDetail(p,lv){return (p.ults||[]).filter((u,i)=>i<4?lv>=ULT_UNLOCK_LEVELS[i]:p.id==='yusha');}
function playerDetailSkills(p,lv,type){const a={...p,level:lv,figureEffects:figureEffectsFor(p.id)};return learnedBattleSkills(a,type);}
function openPlayerDetail(pid){const row=state.party.find(x=>x[0]===pid),p=player(pid),ov=$('#playerDetailOverlay'),body=$('#playerDetailBody');if(!row||!p||!ov||!body)return;const lv=row[1],st=baseStats(p,lv),v=ensureAdventureVitals()[pid],eq=equipmentFor(pid),magic=playerDetailSkills(p,lv,'magic'),tech=playerDetailSkills(p,lv,'special'),ults=learnedUltimatesForDetail(p,lv);body.innerHTML=`<div class="player-detail-hero"><img src="${versionedPlay(p.image)}" alt="${p.name}"><div><small>${p.role||''} / ${p.attribute}</small><h2>${p.name}</h2><b>Lv${lv}</b><em>${statusLabel(v)}</em></div></div><section><h3>ステータス</h3><div class="player-detail-stats"><span>HP <b>${Math.round(v.hp)}/${st.maxHp}</b></span><span>MP <b>${Math.round(v.mp)}/${st.maxMp}</b></span><span>ATK <b>${st.atk}</b></span><span>MAG <b>${st.mag}</b></span><span>DEF <b>${st.def}</b></span><span>MND <b>${st.res}</b></span><span>SPD <b>${st.spd}</b></span></div></section><section><h3>魔法</h3>${magic.length?`<div class="player-detail-list">${magic.map(x=>`<span><b>${x.name}</b><small>MP ${x.cost} / ${x.desc}</small></span>`).join('')}</div>`:'<p>現在使用できる魔法はありません。</p>'}</section><section><h3>特技</h3>${tech.length?`<div class="player-detail-list">${tech.map(x=>`<span><b>${x.name}</b><small>MP ${x.cost} / ${x.desc}</small></span>`).join('')}</div>`:'<p>現在使用できる特技はありません。</p>'}</section><section><h3>装備</h3><div class="player-detail-list"><span><b>MAIN</b><small>${eq.main?weaponById(eq.main)?.name||eq.main:'なし'}</small></span><span><b>SUB</b><small>${eq.sub?weaponById(eq.sub)?.name||eq.sub:'なし'}</small></span><span><b>FIGURE</b><small>${figureEquipmentFor(pid).filter(Boolean).map(id=>figureById(id)?.name||id).join(' / ')||'なし'}</small></span></div></section><section><h3>必殺技</h3><div class="player-detail-list">${ults.map((u,i)=>`<span><b>${u.name}</b><small>CT ${ultimateEffectiveCt({...p,figureEffects:figureEffectsFor(pid)},u,(p.ults||[]).indexOf(u))} / ${u.desc}</small></span>`).join('')||'<span><b>なし</b></span>'}</div></section>`;bindImages(body);ov.hidden=false;}
...
function renderCastle(){castleView='menu';setCastleBackground('back2/003.png','back/rpgmain.png');setCastleHeader('CASTLE','お城','FACILITIES');const root=$('#castleContent');root.className='page-scroll nav-spacer castle-content castle-menu-view';root.innerHTML=`<section class="castle-menu-stage"><div class="castle-title-card"><small>CASTLE FACILITIES</small><h2>お城</h2><p>利用する施設を選んでください。</p></div><div class="castle-main-icons"><button class="castle-slot top" data-castle-menu="throne" type="button"><img src="icon/18.png" alt="王の間"><b>王の間</b></button><button class="castle-slot top" data-castle-menu="inn" type="button"><img src="icon/19.png" alt="宿舎"><b>宿舎</b></button><button class="castle-slot bottom" data-castle-menu="shop" type="button"><img src="icon/20.png" alt="MOB SHOP"><b>MOB SHOP</b></button><button class="castle-slot bottom" data-castle-menu="records" type="button"><img src="icon/21.png" alt="レコードルーム"><b>レコードルーム</b><small>LOCKED</small></button><button class="castle-slot bottom" data-castle-menu="smith" type="button"><img src="icon/23.png" alt="鍛冶屋"><b>鍛冶屋</b></button></div></section>`;bindImages(root);bindCastleContentEvents();}
...
async function submitAdventureReport(){if(castleReportBusy)return;const r=state.adventure.awaitingReport;if(!r)return showCastleSpeech('モブスライムキング','今は新しい報告はないようじゃな',document.querySelector('[data-castle-actor="king"]'),'center');castleReportBusy=true;try{await runCastleReportEvent(r.worldId);if(!CASTLE_REPORT_EVENTS[r.worldId]){const worlds=MOB_DATA.adventureWorlds||[],w=worlds[r.worldIndex]||currentWorld();await facilityTalk(`${w?.name||'今回の地'}の報告、ご苦労じゃ！\nよくやった！`,'モブスライムキング','play/007.png');}finishAdventureReportProgress(r);await fixedDelay(1000);renderCastle();showScreen('castle');}finally{castleReportBusy=false;}}
function castleActorSpeak(kind,actorEl){if(castleReportBusy||storyBusy||!$('#dialogOverlay').hidden)return;if(kind==='king'&&state.adventure.awaitingReport)return submitAdventureReport();if(kind==='king')showCastleSpeech('モブスライムキング',pick(['頼むぞ、運命はお主たちにかかっている！','時には休息も大事じゃぞ！','装備は整っておるか？','城の設備はどんどん使ってくれ！']),actorEl,'center');else showCastleSpeech('モブライトアーム',pick(['みなさん、お気をつけて','ここはお任せを！','城は私が守ります！']),actorEl,'left');}
...
function renderBlacksmithPopup(mode='menu'){const pop=$('#blacksmithPopup'),body=$('#blacksmithPopupBody'),title=$('#blacksmithPopupTitle');if(!pop||!body)return;blacksmithView=mode;title.textContent=mode==='shop'?'武器購入':mode==='forge'?'メダル錬成':'鍛冶屋';if(mode==='menu')body.innerHTML=`<div class="blacksmith-popup-menu"><button data-blacksmith-popup-action="shop" type="button">武器購入</button><button data-blacksmith-popup-action="forge" type="button">メダル錬成</button></div>`;else body.innerHTML=`<button class="blacksmith-popup-back" data-blacksmith-popup-back type="button">← 選択へ戻る</button><div class="blacksmith-popup-scroll blacksmith-list">${blacksmithWeaponList(mode)}</div>`;bindImages(body);}
function openBlacksmithPopup(mode='menu'){renderBlacksmithPopup(mode);$('#blacksmithPopup').hidden=false;}
function closeBlacksmithPopup(){const p=$('#blacksmithPopup');if(p)p.hidden=true;blacksmithView='menu';}
function bindBlacksmithPopupEvents(){const pop=$('#blacksmithPopup');if(!pop)return;pop.onclick=e=>{if(e.target===pop||e.target.closest('[data-blacksmith-popup-close]'))return closeBlacksmithPopup();const a=e.target.closest('[data-blacksmith-popup-action]');if(a)return renderBlacksmithPopup(a.dataset.blacksmithPopupAction);if(e.target.closest('[data-blacksmith-popup-back]'))return renderBlacksmithPopup('menu');const bw=e.target.closest('[data-buy-blacksmith-weapon]');if(bw)return buyWeapon(bw.dataset.buyBlacksmithWeapon,()=>renderBlacksmithPopup('shop'));const fm=e.target.closest('[data-forge-blacksmith]');if(fm)return forgeWeaponMedal(fm.dataset.forgeBlacksmith,()=>renderBlacksmithPopup('forge'));};}
...
function renderBlacksmithRoom(mode='menu'){castleView='smith';blacksmithView='menu';setCastleBackground('back/gonzo.png','back2/003.png');setCastleHeader('BLACKSMITH','鍛冶屋',`${state.coins.toLocaleString()} G`);const root=$('#castleContent');root.className='page-scroll nav-spacer castle-content castle-room-view blacksmith-room-view';root.innerHTML=`<section class="castle-room-stage blacksmith-stage-v68"><button class="blacksmith-gonzo" data-blacksmith-host type="button"><img src="play/002.png" alt="モブゴンゾー"><b>モブゴンゾー</b><small>タップして利用</small></button>${castleHomeButton('blacksmith-home-v68')}</section>`;bindImages(root);bindCastleContentEvents();}
async function enterBlacksmithRoom(){renderBlacksmithRoom();await facilityIntro('smith:v68',{speaker:'モブゴンゾー',image:'play/002.png',first:`よう！よく来たな！\nここでは装備の購入と\nメダルの錬成が出来るぞ！\n装備は武器と防具に分かれていて\n武器は2つまで、防具は1つ装備出来るぞ！\n武器の2つ目はサブ武器でステータスが半減する\n注意して装備してくれ！\n同じ武器を3つ持ってきたらメダル錬成が出来るぞ！\nメダルはその武器のステータス10％と、\nなんと特性を引き継ぐことが出来るぞ！\nメダルにした武器は消えてしまうから注意してくれ！`,repeat:'よう！今日はどうする？'});}
...
function bindCastleContentEvents(){
  ...
  root.onclick=e=>{
    ...
    if(e.target.closest('[data-blacksmith-host]'))return openBlacksmithPopup('menu');
  };
}
...
function bindEvents(){
  ...
  $('#inventoryCloseBtn').onclick=closeInventory;$('#playerDetailCloseBtn').onclick=closePlayerDetail;$('#playerDetailOverlay').addEventListener('click',e=>{if(e.target===$('#playerDetailOverlay'))closePlayerDetail();});bindBlacksmithPopupEvents();$('#inventoryOverlay').addEventListener('click',e=>{if(e.target===$('#inventoryOverlay'))closeInventory();});$$('[data-inventory-tab]').forEach(b=>b.onclick=()=>{inventoryTab=b.dataset.inventoryTab;renderInventory();});
...
function applyV68StoryCorrections(){
  const replaceText=(oldText,newText)=>{for(const ev of Object.values(STORY_EVENTS||{}))for(const st of ev.steps||[])if(typeof st?.[2]==='string'&&st[2]===oldText)st[2]=newText;};
  replaceText('魔王を討伐すべく、流れるように旅をしているでござる！','魔王を討伐すべく旅をしているでござる！');
  replaceText(`この世界支配するの\n魔王様だ\nお前たちに\n邪魔はさせない`,`この世界を支配するのは魔王様\nお前たちに\n邪魔はさせない`);
  replaceText('恐ろしい海賊でありました・・','恐ろしい海賊でありました');replaceText('雷のモンスターか・・！','雷のモンスターか！');replaceText('どんな理由でも\nあの町の住人は帰ってこない・・！','どんな理由でも\nあの町の住人は帰ってこない！！');
  replaceText('不気味な妖気を感じるでござる','妖気を感じるでござる');replaceText('怖いでやんす・・','(怖いでやんす)');replaceText('こんなことが・・','なんだこれは・・！？');replaceText('とんでもない魔力ね・・！','とんでもない魔力ね！');replaceText('ミラモブ4体分か・・','ミラモブ4体分か');
  const addAfter=(key,actor,text,newStep)=>{const ev=STORY_EVENTS[key];if(!ev?.steps||ev.steps.some(st=>st?.[1]===newStep[1]&&st?.[2]===newStep[2]))return;const i=ev.steps.findIndex(st=>st?.[1]===actor&&st?.[2]===text);if(i>=0)ev.steps.splice(i+1,0,newStep);};
  addAfter('pre:rural:2','pink','見れば分かるであります',['say','money','マヒに注意ね！']);
  addAfter('pre:desert2:1','pink','サポートし合いながら戦いましょう！',['say','money','行くわよ！']);
  const postD2=STORY_EVENTS['post:desert2:1'];if(postD2?.steps&&!postD2.steps.some(st=>st?.[1]==='denden'&&st?.[2]==='やったであります！'))postD2.steps.unshift(['say','denden','やったであります！']);
  replaceText('なんという不気味な魔力だ','なんという不気味な魔力だ');replaceText(`小賢しいガキ共だ・・\n踏みつぶしてくれる！`,`小賢しいガキ共だ\n踏みつぶしてくれる！`);replaceText('貴様ら如きにこの私が・・！','貴様ら如きにこの私が!!');replaceText('ソウル・タイム・ミラー・・！！','ソウル・タイム・ミラー！！');replaceText('そんな・・！','そんな!！');replaceText('結局勝つのは私たちだ・・！','結局勝つのは私たちだ！');
  const d2pre2=STORY_EVENTS['pre:desert2:2'];if(d2pre2?.steps){for(const st of d2pre2.steps){if(st?.[0]==='showGroup'){for(const g of st[1]||[]){const map={'d2-miraearth':'enemy/37.png','d2-mirakarami':'enemy/38.png','d2-miranight':'enemy/39.png','d2-miratime':'enemy/40.png'};if(map[g?.key])g.image=map[g.key];}}}}
}
applyV68StoryCorrections();
