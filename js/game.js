...
function parseFigureStatsText(text){
  const out={maxHp:0,maxMp:0,atk:0,mag:0,def:0,res:0,spd:0};let s=String(text||'').replace(/DFE/g,'DEF');
  s=s.replace(/HP\s*[\.&]\s*MP\s*\+\s*(\d+)/gi,(_,n)=>{out.maxHp+=Number(n);out.maxMp+=Number(n);return' ';});
  const map={HP:'maxHp',MP:'maxMp',ATK:'atk',MAG:'mag',DEF:'def',MND:'res',SPD:'spd'};
  for(const m of s.matchAll(/(HP|MP|ATK|MAG|DEF|MND|SPD)\s*\+\s*(\d+)/g))out[map[m[1]]]+=Number(m[2]);
  return out;
}
...
function parseFigureEffectText(text){
  const out=emptyFigureEffects(),s=String(text||'').replace(/DFE/g,'DEF').replace(/無し/g,'');out.stats=parseFigureStatsText(s);
  let m;
  if((m=s.match(/全属性耐性\s*\+?\s*(\d+)%?/)))for(const k of Object.keys(out.resist))out.resist[k]+=Number(m[1])/100;
  for(const x of s.matchAll(/([火水雷風地光闇無])(?:属性)?(?:ダメージ)?耐性\s*\+?\s*(\d+)%?/g))out.resist[x[1]]+=Number(x[2])/100;for(const x of s.matchAll(/([火水雷風地光闇無])属性ダメージ軽減\s*\+?\s*(\d+)%?/g))out.resist[x[1]]+=Number(x[2])/100;
  const statusMap={'毒':'poison','やけど':'burn','マヒ':'paralyze','眠り':'sleep','ひるみ':'stun'};
  for(const [label,key] of Object.entries(statusMap)){const x=s.match(new RegExp(label+'耐性\\s*\\+?\\s*(\\d+)%?'));if(x)out.statusResist[key]+=Number(x[1])/100;}
  if((m=s.match(/会心率\s*\+?\s*(\d+)%/)))out.crit+=Number(m[1])/100;
  if((m=s.match(/回避率\s*\+?\s*(\d+)%/)))out.evade+=Number(m[1])/100;
  let work=s.replace(/物理ダメージ軽減\s*\+?\s*\d+%?/g,'').replace(/[火水雷風地光闇無]属性ダメージ軽減\s*\+?\s*\d+%?/g,'');
  if((m=s.match(/物理ダメージ軽減\s*\+?\s*(\d+)%?/)))out.physicalCut+=Number(m[1])/100;
  if((m=work.match(/ダメージ軽減\s*\+?\s*(\d+)%?/)))out.damageCut+=Number(m[1])/100;
  if((m=s.match(/戦闘獲得経験値\s*&\s*戦闘獲得コイン\s*\+?\s*(\d+)%/))){out.expBonus+=Number(m[1])/100;out.goldBonus+=Number(m[1])/100;}
  else{if((m=s.match(/獲得経験値\s*\+?\s*(\d+)%/)))out.expBonus+=Number(m[1])/100;if((m=s.match(/(?:戦闘)?獲得コイン\s*\+?\s*(\d+)%/)))out.goldBonus+=Number(m[1])/100;}
  if((m=s.match(/弱点を突いた時のダメージ\s*\+?\s*(\d+)%/)))out.weakDamage+=Number(m[1])/100;
  if((m=s.match(/通常攻撃の与ダメージ\s*\+?\s*(\d+)%/)))out.normalDamage+=Number(m[1])/100;
  if(!/[火水雷風地光闇無]属性魔法与ダメージ/.test(s)&&(m=s.match(/(?:魔法攻撃の与ダメージ|魔法与ダメージ)\s*\+?\s*(\d+)%/)))out.magicDamage+=Number(m[1])/100;
  for(const x of s.matchAll(/([火水雷風地光闇無])属性魔法与ダメージ\s*\+?\s*(\d+)%/g))out.elementMagicDamage[x[1]]+=Number(x[2])/100;
  if((m=s.match(/必殺技ダメージ\s*\+?\s*(\d+)%/)))out.ultimateDamage+=Number(m[1])/100;
  if((m=s.match(/(?:^|&)\s*与ダメージ\s*\+?\s*(\d+)%/)))out.damageBonus=(out.damageBonus||0)+Number(m[1])/100;
  if((m=s.match(/探索のレアアイテム率\s*\+?\s*(\d+)%/)))out.rareExplore+=Number(m[1])/100;
  if((m=s.match(/命中率\s*\+?\s*(\d+)%/)))out.accuracy+=Number(m[1])/100;
  if((m=s.match(/MP消費\s*-\s*(\d+)%/)))out.mpCut+=Number(m[1])/100;
  if((m=s.match(/回復量\s*\+?\s*(\d+)%/)))out.healBoost+=Number(m[1])/100;
  if((m=s.match(/必殺技(?:の)?CT\s*-\s*(\d+)ターン/)))out.ultimateCtCut+=Number(m[1]);
  if((m=s.match(/ネオン街の戦闘で全ステータス\s*\+?\s*(\d+)%/)))out.allStatPercent+=Number(m[1])/100;
  if((m=s.match(/HP\s*30以下でDEF\s*\+?\s*(\d+)/)))out.lowHpDefFlat+=Number(m[1]);
  return out;
}
...
function equipmentDetailMarkup(pid){const p=player(pid),lv=currentPlayerLevel(pid),eq=equipmentFor(pid),raw=rawBaseStats(p,lv),st=weaponStatsForEquipment(p,lv,eq),fake={...p,id:p.id,equipment:eq,maxHp:st.maxHp,hp:st.maxHp,figureEffects:figureEffectsFor(pid)},labels={maxHp:'HP',maxMp:'MP',atk:'ATK',mag:'MAG',def:'DEF',res:'MND',spd:'SPD'};const statRows=Object.entries(labels).map(([k,n])=>{const d=st[k]-raw[k];return`<div><span>${n}</span><b>${st[k]}</b><small>基礎 ${raw[k]}${d?` / 装備 +${d}`:''}</small></div>`;}).join('');const elems=['火','水','雷','風','地','光','闇','無'].map(el=>{const wr=weaponResistance(fake,el)+(el==='闇'?weaponDarkResist(fake):0),fr=figureResistanceTotal(pid,el),effectiveCut=1-(1-clamp(wr,0,.95))*(1-clamp(fr,0,.95));return`<div><span>${el}耐性</span><b>${Math.round(effectiveCut*100)}%</b><small>武器 ${Math.round(wr*100)}% / FIG ${Math.round(fr*100)}%</small></div>`;}).join('');const fe=figureEffectsFor(pid),status=[['poison','毒'],['burn','やけど'],['paralyze','マヒ'],['sleep','眠り'],['stun','ひるみ']].map(([k,n])=>`<div><span>${n}耐性</span><b>${Math.round((.20+Number(fe.statusResist?.[k]||0))*100)}%</b><small>基礎20% + FIG/共鳴 ${Math.round(Number(fe.statusResist?.[k]||0)*100)}%</small></div>`).join(''),crit=TEMP_BALANCE.critRate+weaponCritBonus(fake)+Number(fe.crit||0),evade=weaponEvasion(fake)+Number(fe.evade||0),acc=1+weaponTraitSum(fake,'accuracy')+Number(fe.accuracy||0),misc=[`会心率 ${Math.round(crit*100)}%`,`基本命中 100% / 補正 ${Math.round((acc-1)*100)>=0?'+':''}${Math.round((acc-1)*100)}%`,`回避率 ${Math.round(evade*100)}%`,`物理軽減 ${Math.round((weaponPhysicalCut(fake)+Number(fe.physicalCut||0))*100)}%`,`ダメージ軽減 ${Math.round(Number(fe.damageCut||0)*100)}%`,`回復量 +${Math.round(Number(fe.healBoost||0)*100)}%`,`EXP +${Math.round(Number(fe.expBonus||0)*100)}%`,`コイン +${Math.round(Number(fe.goldBonus||0)*100)}%`,`必殺CT -${Number(fe.ultimateCtCut||0)}ターン`];return`<div class="status-detail-grid">${statRows}</div><h3>属性耐性</h3><div class="status-detail-grid resist">${elems}</div><h3>状態異常耐性</h3><div class="status-detail-grid resist">${status}</div><h3>戦闘特性</h3><div class="status-detail-misc">${misc.map(x=>`<span>${x}</span>`).join('')}</div>${figureResonanceMarkup(pid,true)}`;}
...
function grantTestItemsMax(){
  ...
  if(!state.meta.figures)state.meta.figures={};if(!Array.isArray(state.meta.figureOrder))state.meta.figureOrder=[];for(const f of FIGURES){state.meta.figures[f.id]=99;if(!state.meta.figureOrder.includes(f.id))state.meta.figureOrder.push(f.id);}
  saveMeta();
