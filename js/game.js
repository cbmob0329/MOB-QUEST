...
async function leaveTraining(){await showFacilityExit('play/003.png','また来てくれよな！\nレッツトレーニング！','blue');await goHome();}
...
function areaCampRecord(){if(!state.adventure.campUsed||typeof state.adventure.campUsed!=='object')state.adventure.campUsed={};const key=currentAreaKey(),raw=state.adventure.campUsed[key];if(raw===true){state.adventure.campUsed[key]={tent:true,chair:true,drink:true};}else if(!raw||typeof raw!=='object'){state.adventure.campUsed[key]={tent:false,chair:false,drink:false};}return state.adventure.campUsed[key];}
function areaCampUsed(action=null){const r=areaCampRecord();if(action)return !!r[action];return ['tent','chair','drink'].every(k=>!!r[k]);}
function areaCampAnyUsed(){const r=areaCampRecord();return ['tent','chair','drink'].some(k=>!!r[k]);}
function markAreaCampUsed(action){if(!action)return;const r=areaCampRecord();r[action]=true;saveAdventure();}
...
function renderCampMain(){const r=areaCampRecord();$('#campTitle').textContent=`キャンプ / ${currentArea()?.name||''}`;$('#campUsageText').textContent='テント・椅子・ドリンクはこのAREAでそれぞれ1回使用できます';$('#campTentCount').textContent=tentCount();$('#campMainMenu').hidden=false;$('#campSubPanel').hidden=true;$$('[data-camp-action="tent"],[data-camp-action="chair"],[data-camp-action="drink"]',$('#campMainMenu')).forEach(b=>b.classList.toggle('camp-used',!!r[b.dataset.campAction]));bindImages($('#campOverlay'));}
...
async function useCampTent(){if(areaCampUsed('tent'))return dialog('このAREAではすでにテントを使用しました。',[['OK','ok']],'SYSTEM');if(tentCount()<1)return dialog('モブテントを所持していません！',[['OK','ok']],'SYSTEM');const ans=await dialog('テントで休みますか？\nモブテントを1つ消費します',[['はい','yes','primary'],['いいえ','no']],'SYSTEM');if(ans!=='yes')return;if(!consumeItem('mob-tent',1))return;await campFadeMessage('パーティーのHPとMPが全回復した！',async()=>{healCampVitals(1);markAreaCampUsed('tent');saveCampCheckpoint();});renderCampMain();renderAdventure();}
async function useCampChair(){if(areaCampUsed('chair'))return dialog('このAREAではすでに椅子を使用しました。',[['OK','ok']],'SYSTEM');const ans=await dialog('椅子で休みますか？',[['はい','yes','primary'],['いいえ','no']],'SYSTEM');if(ans!=='yes')return;await campFadeMessage('パーティーのHPとMPが少し回復した！',async()=>{healCampVitals(.30);markAreaCampUsed('chair');saveCampCheckpoint();});renderCampMain();renderAdventure();}
...
function openPlayerDetail(pid){const p=player(pid),entry=state.party.find(x=>x[0]===pid);if(!p||!entry)return;let ov=document.getElementById('playerDetailOverlay');if(!ov){ov=document.createElement('div');ov.id='playerDetailOverlay';ov.className='player-detail-overlay';document.body.appendChild(ov);}const lv=entry[1],st=baseStats(p,lv),eq=equipmentFor(pid),magic=(p.magic||[]).filter(x=>!x.level||x.level<=lv),special=(p.specials||[]).filter(x=>!x.level||x.level<=lv),ults=(p.ults||[]).filter((u,i)=>lv>=[1,15,30,50][i]);ov.innerHTML=`<section class="player-detail-card"><button data-detail-close type="button">×</button><header><img src="${versionedPlay(p.image)}" alt="${p.name}"><div><small>CHARACTER STATUS</small><h2>${p.name}</h2><b>Lv${lv}</b></div></header><div class="player-detail-stats"><span>HP <b>${st.maxHp}</b></span><span>MP <b>${st.maxMp}</b></span><span>ATK <b>${st.atk}</b></span><span>DEF <b>${st.def}</b></span><span>MAG <b>${st.mag}</b></span><span>MND <b>${st.res}</b></span><span>SPD <b>${st.spd}</b></span></div><div class="player-detail-section"><h3>魔法</h3><p>${magic.length?magic.map(x=>x.name).join(' / '):'なし'}</p></div><div class="player-detail-section"><h3>特技</h3><p>${special.length?special.map(x=>x.name).join(' / '):'なし'}</p></div><div class="player-detail-section"><h3>装備</h3><p>${[eq.main&&weaponById(eq.main)?.name,eq.sub&&weaponById(eq.sub)?.name].filter(Boolean).join(' / ')||'なし'}</p></div><div class="player-detail-section"><h3>必殺技</h3><p>${ults.length?ults.map(x=>x.name).join(' / '):'なし'}</p></div></section>`;bindImages(ov);ov.classList.add('show');ov.querySelector('[data-detail-close]').onclick=()=>ov.classList.remove('show');ov.onclick=e=>{if(e.target===ov)ov.classList.remove('show');};}
...
function initUltimateCooldowns(a){const base=(a.ults||[]).map((u,i)=>ultimateEffectiveCt(a,u,i)),stored=state.meta?.ultimateCooldowns?.[a.id];a.ultCooldowns=Array.isArray(stored)?base.map((v,i)=>stored[i]==null?v:Math.max(0,Number(stored[i])||0)):base;}
function persistUltimateCooldownsFromBattle(){const b=state.battle;if(!b?.allies)return;if(!state.meta.ultimateCooldowns)state.meta.ultimateCooldowns={};for(const a of b.allies){if(Array.isArray(a.ultCooldowns))state.meta.ultimateCooldowns[a.id]=a.ultCooldowns.map(v=>Math.max(0,Number(v)||0));}saveMeta();}
...
function enemyVisualTune(e){
  ...
  if(id==='lilith'||id==='boss-lilith-castle'||name==='モブリリス')return{scale:1.20,y:0};
  return{scale:1,y:0};
}
...
async function spawnNextEnemyWave(){
  ...
  if(isD2Revive){await actionCutin('モブマニー「・・・・？\nなんだろう\n嫌な予感がする」','system',1100);await fixedDelay(220);await actionCutin('モブデザート「終わってないのか？」','system',1000);await fixedDelay(220);await storyDarkBattlePulse();const lead=(b.enemies||[]).find(e=>e.id==='d2-miratime')||(b.enemies||[]).find(e=>e.id==='d2-miranight');if(lead)await enemyStoryCutin(lead,'ソウル・タイム・ミラー・・！！',1300);}
...
function persistBattlePartyOrder(){const b=state.battle;if(!b)return;const ids=[...(b.mainIds||[]),...(b.superIds||[]),...(b.reserveIds||[])].filter(Boolean),levelMap=new Map(state.party.map(([id,lv])=>[id,lv])),seen=new Set(),next=[];for(const id of ids){if(seen.has(id)||!levelMap.has(id))continue;seen.add(id);next.push([id,levelMap.get(id)]);}for(const [id,lv] of state.party){if(!seen.has(id)){seen.add(id);next.push([id,lv]);}}if(next.length){state.party=next;state.training.party=next.map(x=>[...x]);saveParty();}}
function finishBattle(win){
  const b=state.battle;if(!b||b.finished)return;if(b.mode==='story')return finishScriptedBattle();persistBattlePartyOrder();persistUltimateCooldownsFromBattle();
...
function castleActorSpeak(kind,actorEl){
  ...
  else showCastleSpeech('モブライトアーム',pick(['みなさんお気をつけて','ここはお任せを！','城は私が守ります！']),actorEl,'left');
}
...
async function openBlacksmithFacility(){
  await facilityIntro('smith',{speaker:'モブゴンゾー',image:'play/002.png',first:`よう！よく来たな！\nここでは装備の購入と\nメダルの錬成が出来るぞ！\n装備は武器と防具に分かれていて\n武器は2つまで、防具は1つ装備出来るぞ！\n武器の2つ目はサブ武器でステータスが半減する\n注意して装備してくれ！\n同じ武器を3つ持ってきたらメダル錬成が出来るぞ！\nメダルはその武器のステータス10％と、\nなんと特性を引き継ぐことが出来るぞ！\nメダルにした武器は消えてしまうから注意してくれ！`,repeat:'よく来たな！\nいい武器手に入れたか？'});
  equipmentFacilityOrigin='smith';equipmentTab='shop';equipmentPlayerId=state.party[0]?.[0]||'yusha';renderEquipment();showScreen('equipment');
}
