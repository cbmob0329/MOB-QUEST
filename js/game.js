...
function renderTraining(){
  ...
  $$('[data-training-member]').forEach(sel=>sel.onchange=()=>{const i=Number(sel.dataset.trainingMember);if(state.training.party.some((x,j)=>j!==i&&x[0]===sel.value)){toast('同じキャラクターは2人編成できません');return renderTraining();}state.training.party[i][0]=sel.value;renderTraining();});$$('[data-training-level]').forEach(i=>i.onchange=()=>{state.training.party[Number(i.dataset.trainingLevel)][1]=clamp(Number(i.value)||1,1,99);});$$('[data-boss-stage]').forEach(b=>b.onclick=()=>{state.training.filter=b.dataset.bossStage;renderTraining();});$$('[data-boss-id]').forEach(b=>b.onclick=()=>{state.training.bossId=b.dataset.bossId;renderTraining();});
}
...
function allyMarkup(a){return`<button type="button" class="ally-hud-card ${a.dead?'dead':''} ${activeAlly()===a?'active turn-active':''}" data-ally-id="${a.id}"><span class="ally-hud-art"><img src="${a.image}" alt="${a.name}"><i>${a.symbol}</i></span><b>${a.name}</b><small>Lv${a.level} ${a.dead?'DOWN':statusText(a)}</small><div class="ally-hud-line"><span>HP ${Math.ceil(a.hp)} / ${a.maxHp}</span><span>MP ${Math.floor(a.mpNow)} / ${a.maxMp}</span></div><div class="ally-gauges"><div class="gauge tiny"><i class="hp" style="width:${pct(a.hp,a.maxHp)}%"></i></div><div class="gauge tiny"><i class="mp" style="width:${pct(a.mpNow,a.maxMp)}%"></i></div></div></button>`;}
...
function renderBattle(){const b=state.battle;if(!b)return;$('#battleTurnLabel').textContent=`TURN ${b.turn}`;$('#enemyArea').innerHTML=enemyMarkup(b.enemy);$('#allyStatus').innerHTML=mainAllies().map(allyMarkup).join('');$('#superStatus').innerHTML=superAllies().length?superAllies().map(superMarkup).join(''):`<div class="no-bench">SUPER SUBなし</div>`;$('#benchStatus').innerHTML=reserveAllies().length?reserveAllies().map(benchMarkup).join(''):`<div class="no-bench">控えなし</div>`;const e=currentEntry(),a=activeAlly();$('#activeActorBar').innerHTML=a?`<img src="${a.image}" alt=""><div><small>COMMAND / SPD ${Math.round(effective('spd',a))}</small><b>${a.name}</b><span>HP ${Math.ceil(a.hp)} / MP ${Math.floor(a.mpNow)}</span></div>`:e?.type==='super'?`<div><small>SUPER SUB AUTO</small><b>${allyById(e.id)?.name||''}</b></div>`:`<div><small>WAIT</small><b>${b.enemy.name}</b></div>`;bindImages($('#battleScreen'));setCommandDisabled(b.busy||b.finished||!a);}
...
function pulseEnemy(cls='hit'){const el=$('#enemySprite')||$('.enemy-symbol');if(!el)return;el.classList.remove('enemy-hit','enemy-cast','enemy-advance');void el.offsetWidth;el.classList.add(cls==='cast'?'enemy-cast':cls==='advance'?'enemy-advance':'enemy-hit');}
...
async function ultimateCutin(a,u){const wrap=$('#ultimateCutin');setImage($('#cutinCharacter'),a.transformed&&a.id==='yusha'?'play/13.png':a.image,'');setImage($('#cutinUltArt'),u.image,'');$('#cutinName').textContent=u.name;$('#cutinQuote').textContent='';$('#cutinUltFallback').textContent=u.name;wrap.hidden=false;wrap.classList.remove('play');void wrap.offsetWidth;wrap.classList.add('play');await delay(1350);wrap.hidden=true;wrap.classList.remove('play');}
...
function applyEnemyDamage(a,power,type='physical',crit=0){const e=state.battle.enemy,r=calcDamage(a,type,power,crit);let d=r.value;if(e.shieldTurns>0)d=Math.round(d*(1-(e.damageReduction||.2)));e.hp=Math.max(0,e.hp-d);floatNumber(d,r.crit?'crit':'damage','enemy');if(r.crit)notice('CRITICAL!','damage',500);fx(type==='magic'?'magic':'slash');pulseEnemy();wakeEnemyOnHit();renderBattle();return{...r,value:d};}
...
async function checkSpecialRevives(){const field=fieldAllies(),pink=field.find(a=>a.id==='pink'&&!a.dead&&!a.pinkReviveUsed);for(const a of field){if(a.dead&&a.id==='lilith'&&!a.lilithReviveUsed){a.dead=false;a.lilithReviveUsed=true;a.transformed=true;a.hp=Math.round(a.maxHp*.60);a.atk*=1.2;a.mag*=1.2;a.def*=1.2;a.res*=1.2;a.spd*=1.2;notice('モブリリスがウルモブリリスへ変身して復活！','system',950);renderBattle();await delay(400);}else if(a.dead&&pink&&!pink.pinkReviveUsed&&a.id!=='pink'){pink.pinkReviveUsed=true;pink.hp=Math.max(1,Math.floor(pink.hp*.5));a.dead=false;a.hp=Math.round(a.maxHp*.35);notice(`支える力！ ${a.name}が復活！`,'heal',900);renderBattle();await delay(400);break;}}}
function maybeArtistCleanse(target){const riro=livingField().find(a=>a.id==='riro');if(riro&&target&&Math.random()<.5){cleanse(target);notice(`${target.name}の状態異常を解除！`,'status');return true;}return false;}
...
async function damageAlly(a,power,type='physical',superHalf=false){
  ...
  const desert=livingField().find(x=>x.id==='desert');if(desert&&Math.random()<.20)d=Math.round(d*.8);
...
async function inflictAllyStatus(a,kind,turns){if(!a||a.dead)return false;const resist=.2; if(Math.random()<resist)return false;a.status[kind]=Math.max(a.status[kind],turns);if(maybeArtistCleanse(a))return false;return true;}

async function performAttack(a,auto=false){await actionCutin(`${a.name}の攻撃！`,'system',480);let crit=a.id==='denden'?.20:TEMP_BALANCE.critRate;let r=applyEnemyDamage(a,1,'physical',crit);notice(`${r.value} DAMAGE`,'damage',500);if(a.id==='tetsu'&&state.battle.enemy.hp>0&&Math.random()<.30){await delay(150);notice('テツの意志！ 2回攻撃！','system',500);const r2=applyEnemyDamage(a,.85,'physical',TEMP_BALANCE.critRate);notice(`${r2.value} DAMAGE`,'damage',450);}await delay(auto?190:260);}
async function performMagic(a,auto=false){const element=normalizeElement(a.attribute),s=MOB_DATA.elements[element];if(a.mpNow<s.cost){notice('MPが足りない！','danger');return false;}a.mpNow-=s.cost;await actionCutin(`${a.name}の${s.spell}！`,'system',560);await skillSprite(s.frames,'enemy');const r=applyEnemyDamage(a,s.power,'magic');notice(`${r.value} DAMAGE`,'damage',500);if(a.id==='jerry'&&element==='雷'&&state.battle.enemy.hp>0&&Math.random()<.5){await delay(130);notice('ダブルサンダー！','system');await skillSprite(s.frames,'enemy');const r2=applyEnemyDamage(a,s.power*.9,'magic');notice(`${r2.value} DAMAGE`,'damage');}await delay(auto?170:240);return true;}
async function performUltimate(a,u){if(a.mpNow<u.cost){notice('MPが足りない！','danger');return false;}a.mpNow-=u.cost;await actionCutin(`${a.name}の必殺技！`,'danger',420);await ultimateCutin(a,u);let total=0,r;const hit=async(power=u.power,type=u.type||'physical',crit=u.crit||0)=>{r=applyEnemyDamage(a,power,type,crit);total+=r.value;await delay(90);return r;};
  switch(u.kind){
    ...
    case'jumanji':await hit();a.atkBuff=.15;a.atkBuffTurns=3;state.battle.enemy.defDebuff=.12;state.battle.enemy.defDebuffTurns=3;notice(`${total} DAMAGE / ATK↑ DEF↓`,'buff');break;
    case'lowHpBurst':{const all=livingField(),avg=all.reduce((s,x)=>s+x.hp/x.maxHp,0)/Math.max(1,all.length);await hit(u.power*(1+(1-avg)*.65),'magic');notice(`${total} DAMAGE`,'damage');break;}
    ...
    case'shieldAttack':await hit();a.guard=.20;a.guardTurns=1;notice(`${total} DAMAGE / GUARD ↑`,'buff');break;
    ...
    case'yushaGuardAttack':await hit();state.battle.yushaGuard=.50;state.battle.yushaGuardTurns=1;notice(`${total} DAMAGE / 勇者 DAMAGE CUT`,'buff');break;
    case'teamGuardAttack':await hit();state.battle.teamGuard=.30;state.battle.teamGuardTurns=1;notice(`${total} DAMAGE / PARTY GUARD`,'buff');break;
    case'selfHealAttack':heal(a,a.maxHp*.16);await hit();notice(`${total} DAMAGE / HP RECOVER`,'heal');break;
    case'goldAttack':await hit();notice(`${total} DAMAGE`,'damage');break;
    case'speedDebuffAttack':await hit();state.battle.enemy.spdDebuff=.15;state.battle.enemy.spdDebuffTurns=3;notice(`${total} DAMAGE / SPD ↓↓`,'status');break;
    case'burnAttack':await hit();if(applyBossStatus('burn',u.chance||.1))notice(`${state.battle.enemy.name}はやけど状態！`,'status');else notice(`${total} DAMAGE`,'damage');break;
    case'teamDefAttack':await hit();livingField().forEach(x=>{x.defBuff=.15;x.defBuffTurns=3;});notice(`${total} DAMAGE / PARTY DEF ↑`,'buff');break;
    case'selfCleanseAttack':cleanse(a);await hit();notice(`${total} DAMAGE / 状態異常解除`,'status');break;
    case'sleepAttack':await hit();if(applyBossStatus('sleep',u.chance||.1))notice(`${state.battle.enemy.name}は眠った！`,'status');else notice(`${total} DAMAGE`,'damage');break;
    case'paralyzeAttack':await hit();if(applyBossStatus('paralyze',u.chance||.1))notice(`${state.battle.enemy.name}はマヒした！`,'status');else notice(`${total} DAMAGE`,'damage');break;
    case'selfSpdAttack':await hit();a.spdBuff=.18;a.spdBuffTurns=3;notice(`${total} DAMAGE / SPD ↑`,'buff');break;
    case'multiAttack':{const n=rint(u.hits?.[0]||3,u.hits?.[1]||6);for(let i=0;i<n&&state.battle.enemy.hp>0;i++)await hit();notice(`${n} HIT / ${total} DAMAGE`,'damage');break;}
    ...
    case'stunAttack':await hit();if(applyBossStatus('stun',u.chance||.1,1))notice(`${state.battle.enemy.name}をひるませた！`,'status');else notice(`${total} DAMAGE`,'damage');break;
    case'selfRecoveryAttack':await hit();heal(a,a.maxHp*.12);a.mpNow=Math.min(a.maxMp,a.mpNow+a.maxMp*.08);notice(`${total} DAMAGE / HP・MP RECOVER`,'heal');break;
    ...
    case'teamAtkAttack':await hit();livingField().forEach(x=>{x.atkBuff=.15;x.atkBuffTurns=3;});notice(`${total} DAMAGE / PARTY ATK ↑`,'buff');break;
    case'healAttack':healField(u.heal||.24);await hit();notice(`${total} DAMAGE / PARTY HP RECOVER`,'heal');break;
    case'tetsuFinal':a.atkBuff=.18;a.atkBuffTurns=3;state.battle.enemy.defDebuff=.15;state.battle.enemy.defDebuffTurns=3;await hit();notice(`${total} DAMAGE / ATK↑ DEF↓`,'buff');break;
    case'healStunAttack':healField(u.heal||.25);restoreMpField(.10);await hit();if(applyBossStatus('stun',u.chance||.3,1))notice(`${total} DAMAGE / ひるみ！`,'status');else notice(`${total} DAMAGE / PARTY RECOVER`,'heal');break;
    case'poisonAttack':await hit();if(applyBossStatus('poison',u.chance||.3))notice(`${state.battle.enemy.name}は毒になった！`,'status');else notice(`${total} DAMAGE`,'damage');break;
    ...
    case'selfAtkAttack':a.atkBuff=.18;a.atkBuffTurns=3;await hit();notice(`${total} DAMAGE / ATK ↑`,'buff');break;
    case'damage':default:await hit();notice(`${total} DAMAGE`,'damage');break;
  }
  renderBattle();await delay(250);return true;
}

async function applyRoundDots(){const e=state.battle.enemy;if(e.status.poison>0){const d=Math.max(1,Math.round(e.maxHp*.025));e.hp=Math.max(0,e.hp-d);e.status.poison--;floatNumber(d,'damage');notice(`毒のダメージ ${d}`,'status');}if(e.status.burn>0){const d=Math.max(1,Math.round(e.maxHp*.025));e.hp=Math.max(0,e.hp-d);e.status.burn--;floatNumber(d,'damage');notice(`やけどのダメージ ${d}`,'status');}for(const a of fieldAllies()){if(a.dead)continue;for(const k of ['poison','burn'])if(a.status[k]>0){const d=Math.max(1,Math.round(a.maxHp*.025));a.hp=Math.max(0,a.hp-d);a.status[k]--;if(a.hp<=0){a.dead=true;notice(`${a.name} DOWN`,'danger');}else notice(`${a.name}に${k==='poison'?'毒':'やけど'}ダメージ ${d}`,'status');}}renderBattle();await checkSpecialRevives();}
...
async function startRound(){const b=state.battle;if(!b||b.finished)return;b.busy=true;b.queuePos=0;notice(`TURN ${b.turn}`,'system',450);await applyRoundDots();if(b.enemy.hp<=0)return finishBattle(true);if(!livingRoster().length)return finishBattle(false);await resolveRequiredReplacements();if(!livingRoster().length)return finishBattle(false);
  for(const a of fieldAllies().filter(x=>!x.dead)){
    if(a.id==='nekoku'&&Math.random()<.30){const target=[...livingField()].sort((x,y)=>x.hp/x.maxHp-y.hp/y.maxHp)[0];if(target){const h=heal(target,target.maxHp*.14);if(h)notice(`癒しのプニプニ！ +${h} HP`,'heal');}}
    if(a.id==='money'&&Math.random()<.30){const m=Math.round(a.maxMp*.12);a.mpNow=Math.min(a.maxMp,a.mpNow+m);notice(`マニーは海を渡る！ MP +${m}`,'heal');}
    if(a.id==='naraku'){a.narakuStacks++;a.allBuff=Math.min(.80,a.narakuStacks*.10);a.allBuffTurns=99;notice(`魔王の系譜！ ALL STATUS ↑${a.narakuStacks*10}%`,'buff');}
  }
  b.queue=[...livingMain().map(a=>({type:'ally',id:a.id})),{type:'enemy'},...livingSuper().filter(a=>b.turn>=a.nextSupportTurn).map(a=>({type:'super',id:a.id}))].sort((x,y)=>initiativeSpeed(y)-initiativeSpeed(x)+((Math.random()-.5)*.01));b.busy=false;renderBattle();await processQueue();
}
async function processQueue(){const b=state.battle;if(!b||b.finished||b.busy)return;while(b.queuePos<b.queue.length){const e=b.queue[b.queuePos];if(e.type==='ally'){const a=allyById(e.id);if(!a||a.dead||!b.mainIds.includes(a.id)){b.queuePos++;continue;}if(a.status.sleep>0){a.status.sleep--;notice(`${a.name}は眠っている！`,'status');b.queuePos++;await delay(300);continue;}if(a.status.stun>0){a.status.stun--;notice(`${a.name}はひるんで動けない！`,'status');b.queuePos++;await delay(300);continue;}if(a.status.paralyze>0){notice(`${a.name}はマヒして動けない！`,'status');b.queuePos++;await delay(300);continue;}renderBattle();if(b.auto)setTimeout(autoAct,100);return;}
    if(e.type==='enemy'){b.busy=true;renderBattle();await enemyAction();b.busy=false;b.queuePos++;if(b.finished)return;await resolveRequiredReplacements();if(b.finished)return;continue;}
...
async function enemyAction(){const b=state.battle,e=b.enemy;if(e.hp<=0)return finishBattle(true);if(e.status.sleep>0){e.status.sleep--;notice(`${e.name}は眠っている！`,'status');await delay(350);return;}if(e.status.stun>0){e.status.stun--;notice(`${e.name}はひるんで動けない！`,'status');await delay(350);return;}if(e.status.paralyze>0){e.status.paralyze--;notice(`${e.name}はマヒして動けない！`,'status');await delay(350);return;}const special=e.isBoss&&b.turn%TEMP_BALANCE.bossSpecialEvery===0;if(special)await bossSpecial();else await bossNormal();if(!livingRoster().length)finishBattle(false);}
async function bossNormal(){const e=state.battle.enemy,t=pick(livingMain());if(!t)return;await actionCutin(`${e.name}の攻撃！`,'danger',520);pulseEnemy('advance');await delay(430);const d=await damageAlly(t,1,'physical',false);notice(`${t.name} ${d} DAMAGE`,'damage');await delay(320);}
...
async function bossSpecial(){const e=state.battle.enemy;await actionCutin(`${e.name}の${e.special}！`,'danger',700);pulseEnemy('advance');await delay(430);let t,d,total=0;const hit=async(target,m=e.power||1.5,type='physical')=>{const x=await damageAlly(target,m,type,false);await delay(80);return x;};
  switch(e.kind){
    case'shield':e.damageReduction=.20;e.shieldTurns=3;fx('buff');notice('DAMAGE CUT 20%','buff');break;
    case'poisonSingle':t=pick(livingMain());if(t){d=await hit(t);if(Math.random()<.5&&await inflictAllyStatus(t,'poison',3))notice(`${t.name}は毒になった！`,'status');else notice(`${t.name} ${d} DAMAGE`,'damage');}break;
    case'burnSingle':t=pick(livingMain());if(t){d=await hit(t,e.power,'magic');if(Math.random()<.5&&await inflictAllyStatus(t,'burn',3))notice(`${t.name}はやけど状態！`,'status');else notice(`${t.name} ${d} DAMAGE`,'damage');}break;
    case'stunSingle':t=pick(livingMain());if(t){d=await hit(t,e.power,'magic');await inflictAllyStatus(t,'stun',1);notice(`${t.name} ${d} DAMAGE / ひるみ`,'status');}break;
    case'doubleSingleStun':t=pick(livingMain());if(t){total+=await hit(t,e.power,'magic');if(!t.dead)total+=await hit(t,e.power,'magic');await inflictAllyStatus(t,'stun',1);notice(`${t.name} ${total} DAMAGE / ひるみ`,'status');}break;
    case'singlePlusAoe':t=pick(livingMain());if(t)total+=await hit(t,e.power,'magic');total+=await aoeHit(.52,'magic');notice(`TOTAL ${total} DAMAGE`,'damage');break;
    case'multi':case'multiFixed':{const n=rint(e.hits?.[0]||3,e.hits?.[1]||6);for(let i=0;i<n&&livingMain().length;i++)total+=await hit(pick(livingMain()),e.power);notice(`${n} HIT / ${total} DAMAGE`,'damage');break;}
    case'healSingle':t=pick(livingMain());if(t)d=await hit(t,e.power,'magic');{const h=Math.round(e.maxHp*.06);e.hp=Math.min(e.maxHp,e.hp+h);floatNumber(h,'heal');notice(`${d||0} DAMAGE / BOSS HP +${h}`,'heal');}break;
    case'buffAoe':e.atkBuff=.18;e.atkBuffTurns=3;total=await aoeHit(e.power,'magic');notice(`ATK ↑ / TOTAL ${total} DAMAGE`,'buff');break;
    case'doubleAoe':for(let n=0;n<2;n++)total+=await aoeHit(e.power,'physical');notice(`2 HIT / TOTAL ${total} DAMAGE`,'damage');break;
    case'aoeStun':total=await aoeHit(e.power,'magic');for(const a of livingMain())if(Math.random()<.7)await inflictAllyStatus(a,'stun',1);for(const a of livingSuper())if(Math.random()<.7)await inflictAllyStatus(a,'stun',1);notice(`TOTAL ${total} DAMAGE / ひるみ判定`,'status');break;
    case'aoe':total=await aoeHit(e.power,e.attribute.includes('火')||e.attribute.includes('闇')?'magic':'physical');notice(`TOTAL ${total} DAMAGE`,'damage');break;
    case'single':default:t=pick(livingMain());if(t){d=await hit(t,e.power||1.55);notice(`${t.name} ${d} DAMAGE`,'damage');}break;
  }
  await checkSpecialRevives();renderBattle();await delay(280);
}

async function superSubAction(a){await actionCutin(`SUPER SUB! ${a.name}の行動！`,'system',650);if(a.status.sleep>0){a.status.sleep--;notice(`${a.name}は眠っている！`,'status');return;}if(a.status.stun>0){a.status.stun--;notice(`${a.name}はひるんで動けない！`,'status');return;}if(a.status.paralyze>0){notice(`${a.name}はマヒして動けない！`,'status');return;}const low=livingField().some(x=>x.hp/x.maxHp<.45);if((a.id==='money'||a.id==='pink'||a.id==='riro')&&low){const h=healField(.12);notice(`SUPER SUPPORT / PARTY HP +${h}`,'heal');return;}const s=MOB_DATA.elements[normalizeElement(a.attribute)];if(a.mpNow>=s.cost&&Math.random()<.45){await performMagic(a,true);return;}await performAttack(a,true);}
...
async function resolveRequiredReplacements(){const b=state.battle;if(!b||b.finished)return;await checkSpecialRevives();for(let i=0;i<b.mainIds.length;i++){const a=allyById(b.mainIds[i]);if(a&&!a.dead&&a.hp>0)continue;const candidates=replacementCandidates();if(!candidates.length)continue;const inId=await chooseReplacement(`${a?.name||'メイン'}の交代メンバーを選択`,candidates);if(inId){const incoming=allyById(inId);swapGroupMembers(b.mainIds[i],inId);notice(`CHANGE → ${incoming.name}`,'system',750);renderBattle();}}
  for(let i=0;i<b.superIds.length;i++){const a=allyById(b.superIds[i]);if(a&&!a.dead&&a.hp>0)continue;const candidates=reserveReplacementCandidates();if(!candidates.length)continue;const inId=await chooseReplacement(`SUPER SUB ${i+1}を入れ替えますか？`,candidates);if(inId){const incoming=allyById(inId);swapGroupMembers(b.superIds[i],inId);incoming.nextSupportTurn=b.turn+rint(2,5);notice(`SUPER SUB CHANGE → ${incoming.name}`,'system',750);renderBattle();}}
